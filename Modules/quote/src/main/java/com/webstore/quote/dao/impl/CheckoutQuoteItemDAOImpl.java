...
@Repository
public class CheckoutQuoteItemDAOImpl
  extends GenericHibernateDAO<CheckoutQuoteItem, Integer>
  implements CheckoutQuoteItemDAO
{
  @Autowired
  ...
  public List<CheckoutQuoteItem> listCheckoutQuoteItems(int quoteId)
  {
    return 
      ...
      this.sessionFactory.getCurrentSession().createQuery("from CheckoutQuoteItem where quote_id=?").setParameter(0, Integer.valueOf(quoteId)).list();
  }
  ...
  public CheckoutQuoteItem getCheckoutQuoteItem(int quoteId, int productId)
  {
    return 
      ...
      (CheckoutQuoteItem)this.sessionFactory.getCurrentSession().createQuery("from CheckoutQuoteItem where quote_id=? and product_id=?").setParameter(0, Integer.valueOf(quoteId)).setParameter(1, Integer.valueOf(productId)).uniqueResult();
  }
